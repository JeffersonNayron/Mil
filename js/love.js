...
dynamicText.style.top = "70%";
dynamicText.style.left = "50%";
dynamicText.style.transform = "translateX(-50%)";
dynamicText.style.fontSize = "24px";
dynamicText.style.fontFamily = "Arial, sans-serif";
dynamicText.style.color = "#ff4d6d";
dynamicText.style.whiteSpace = "pre";
dynamicText.style.opacity = 0;
dynamicText.style.transition = "opacity 1s ease";
love.parentNode.appendChild(dynamicText);

const frase = "Te amo muito amorzinha, amo por nós dois, ta bom?";
let letraIndex = 0;
...
function escreverTexto() {
    let textTimer = setInterval(() => {
        if (letraIndex < frase.length) {
            dynamicText.textContent += frase[letraIndex];
            letraIndex++;
        } else {
